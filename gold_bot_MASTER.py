...
def fmt_weekly_msg(report: dict, prev: dict = None, label: str = "هذا الأسبوع") -> str:
    ...
    days      = report['days']
    days_done = len(days)
    # أيام التداول في الأسبوع (الاثنين للجمعة = 5)
    now_weekday  = now_local().weekday()
    days_trading = min(now_weekday + 1, 5)  # 5 أيام تداول
    days_left    = max(5 - days_done, 0)
    source_note  = "📡 بيانات مباشرة" if report.get('source') == 'live' else "💾 من قاعدة البيانات"

    lines = [
        f"╔══ 📅 التقرير الأسبوعي — {label} ══╗",
        f"📆 {report['week_start']} → {report['week_end']}",
        f"📊 {days_done}/5 أيام مسجّلة {'✅' if days_done == 5 else f'⏳ ({days_left} باقي)'}",
        f"{source_note}",
        f"",
        f"📊 *أسعار الأيام:*",
        f"```",
        f"{'اليوم':<9} {'فتح':>7} {'إغلاق':>7} {'تغيير':>7}",
        f"{'─'*34}",
    ]
    ...
    for d in days:
        ...
        lines.append(
            ...
            f"{d['open']:>7.1f} "
            f"{d['close']:>7.1f} "
            f"{sign}{d['change']:>5.1f} {arrow}"
        )

    # لو في أيام باقية اليوم الجاري
    if days_done < 5:
        lines.append(f"{'─'*34}")
        lines.append(f"⏳ {days_left} {'يوم باقي' if days_left > 1 else 'يوم باقي'} لإكمال الأسبوع")

    lines += [
        f"```",
        f"",
        f"📈 *ملخص الأسبوع حتى الآن:*",
