...
def detect_order_blocks(d: dict, lookback: int = 30) -> list:
    ...
    for i in range(1, n-2):
        ...
        if c_arr[i] < o_arr[i] and c_arr[i+1] > o_arr[i+1] and move > body1.5:
...
def fmt_analysis_msg(sig: dict, tf: str = '1m') -> str:
    ...
    tp1 = p + atr1.5 if is_bull else p - atr1.5
    tp2 = p + atr3   if is_bull else p - atr3
...
def fmt_pivots_msg(pivots: dict, fib: dict, price: float) -> str:
    ...
    lines = [
        ...
        "",
        "",
        f"R3: {f(pivots['R3'])}  (+{(pivots['R3']-price)/price100:.2f}%)",
        f"R2: {f(pivots['R2'])}  (+{(pivots['R2']-price)/price100:.2f}%)",
        f"R1: {f(pivots['R1'])}  (+{(pivots['R1']-price)/price100:.2f}%)",
        f"PP: {f(pivots['PP'])}  ({(pivots['PP']-price)/price100:+.2f}%)",
        f"S1: {f(pivots['S1'])}  ({(pivots['S1']-price)/price100:.2f}%)",
        f"S2: {f(pivots['S2'])}  ({(pivots['S2']-price)/price100:.2f}%)",
        f"S3: {f(pivots['S3'])}  ({(pivots['S3']-price)/price100:.2f}%)",
        "",
        "",
...
def generate_chart(d: dict, sig: dict, tf: str = '1H') -> Optional[bytes]:
    ...
    try:
        ...
        dire = sig.get('direction', 'NEUTRAL')
        tp1  = price + atr1.5 if dire=='BULLISH' else price - atr1.5
...
async def send_daily_report(context: ContextTypes.DEFAULT_TYPE):
    ...
    if sig['direction'] != 'NEUTRAL':
        ...
        report += [
            f"   {'🟢 TP1' if is_b else '🔴 TP1'}: {fmt_price(p + a1.5 if is_b else p - a1.5)}",
            f"   {'🟢 TP2' if is_b else '🔴 TP2'}: {fmt_price(p + a3   if is_b else p - a3)}",
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "price":
        ...
        if not d:
            ...
            ss   = sig.get("sellScore", 0)
            tp1  = price + atr1.5 if dire == "BULLISH" else price - atr1.5
            tp2  = price + atr3   if dire == "BULLISH" else price - atr3
            sl   = price - atr     if dire == "BULLISH" else price + atr
        ...
        if not d or not d.get("high"):
            ...
            C = d["close"][-1]
            PP = (H+L+C)/3
            R1,R2,R3 = 2PP-L, PP+(H-L), H+2(PP-L)
            S1,S2,S3 = 2PP-H, PP-(H-L), L-2(H-PP)
            rng  = H - L
            f382 = H - rng0.382
            f500 = H - rng0.500
            f618 = H - rng0.618
